  ...
  return (
    <Sheet open onClose={onClose} title={product.name}>
      <div className="space-y-5">
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            ...
            <p className="truncate text-xs text-slate-400">
              {canManage && `Costo ${cop(product.cost)} · `}IVA {product.ivaRate}% · {product.barcode || product.internalCode}
            </p>
          </div>
          {canManage && (
        ...
        {/* Costo promedio, utilidad, sección, último proveedor (costo/utilidad: solo quien gestiona inventario) */}
        <div className="grid grid-cols-2 gap-2 text-sm">
          {canManage && <div className="rounded-lg bg-slate-50 p-2"><span className="text-xs text-slate-400">Costo promedio</span><p className="font-semibold text-slate-700">{cop(product.avgCost ?? product.cost)}</p></div>}
          {canManage && <div className="rounded-lg bg-slate-50 p-2"><span className="text-xs text-slate-400">Utilidad</span><p className="font-semibold text-emerald-600">{product.price > 0 ? Math.round((1 - (product.avgCost ?? product.cost) / product.price) * 100) : 0}%</p></div>}
